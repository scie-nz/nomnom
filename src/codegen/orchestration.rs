...
pub fn generate_all_from_config(config: &GenerationConfig) -> Result<(), Box<dyn Error>> {
    ...
    writeln!(rust_output, "use crate::entity::{{Hl7Entity, ParsingContext, EntityError, FieldValue, IntoOptionString}};")?;
    writeln!(rust_output, "use hl7utils::{{Segment, FieldPath, safe_extract}};")?;
...
fn generate_python_rust_shim(entities: &[EntityDef], extension_module: &str) -> String {
    ...
    // Always include Database and transform functions
    code.push_str("    # Database\n");
    code.push_str("    Database,\n\n");
    code.push_str("    # Transform functions\n");
    code.push_str("    build_segment_index,\n");
    code.push_str("    extract_from_hl7_segment,\n");
...
fn generate_python_package_init(package_name: &str, extension_module: &str) -> String {
    ...
    // Header
    code.push_str(&format!("\"\"\"\n"));
    code.push_str(&format!("{} - HL7v2 message parsing library\n\n", package_name));
    code.push_str("High-performance HL7v2 message parsing with Rust core.\n");
