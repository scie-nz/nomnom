            ...
            if let Ok(yaml) = serde_yaml::from_str::<EntityWrapper>(&yaml_content) {
                if let Some(persistence) = yaml.entity.persistence {
                    if let Some(db_config) = persistence.database {
                        ...
                        // Insert logic - use column-based values to avoid f64 Insertable issues
                        writeln!(output, "                diesel::insert_into({})", table_name)?;
                        writeln!(output, "                    .values((")?;

                        // Build list of column assignments
                        let mut value_assignments = Vec::new();

                        // Add non-autogenerated primary key if present
                        if let Some((pk_name, is_autogen)) = pk_field {
                            if !is_autogen {
                                value_assignments.push(format!("{}.eq(&instance.{})", pk_name, pk_name));
                            }
                        }

                        // Add all other fields
                        for field in &persistence.field_overrides {
                            // For Copy types (i32, bool), use value directly instead of reference
                            // Note: BigDecimal (Float) is NOT a Copy type, so use reference
                            let is_copy_type = matches!(field.field_type.as_str(), "Integer" | "Boolean");
                            if is_copy_type {
                                value_assignments.push(format!("{}.eq(instance.{})", field.name, field.name));
                            } else {
                                value_assignments.push(format!("{}.eq(&instance.{})", field.name, field.name));
                            }
                        }

                        // Write the value assignments
                        for (i, assignment) in value_assignments.iter().enumerate() {
                            if i < value_assignments.len() - 1 {
