#!/bin/bash
#  Simple code generation script for TPC-H example

cd "$(dirname "$0")"

# Run the build.rs logic directly using a small Rust program
cat > /tmp/tpch_codegen.rs << 'RUST_EOF'
use std::env;
...
fn main() {
    let base_path = PathBuf::from(env::args().nth(1).unwrap());
...
# Compile and run
rustc --edition 2021 -L /Users/bogdanstate/nomnom/target/debug/deps \
  --extern nomnom=/Users/bogdanstate/nomnom/target/debug/libnomnom.rlib \
  --extern serde=/Users/bogdanstate/nomnom/target/debug/deps/libserde-*.rlib \
  --extern serde_yaml=/Users/bogdanstate/nomnom/target/debug/deps/libserde_yaml-*.rlib \
  /tmp/tpch_codegen.rs -o /tmp/tpch_codegen && \
  /tmp/tpch_codegen "$PWD"
