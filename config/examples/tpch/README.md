...
│   ├── order.yaml      # Root entity (Order)
│   ├── orderlineitem.yaml  # Derived entity (OrderLineItem)
│   ├── customer.yaml   # Reference entity (Customer)
...
```
└─ 💾 Order [a1b2c3d4]
   └─ 💾 OrderLineItem [e5f6g7h8]
   └─ 💾 OrderLineItem [i9j0k1l2]
```
...
3. **Type Mapping**: `List[Object]` → `Vec<serde_json::Value>`
